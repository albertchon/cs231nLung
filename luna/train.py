...
tf.app.flags.DEFINE_float("best_val_loss", 0.2, "best val loss so far")
...
tf.app.flags.DEFINE_integer("image_height", 512, "height of each slice in pixels")
tf.app.flags.DEFINE_integer("image_width", 512, "width of each slice in pixels")
...
def main(_):
    ...
    for i in range(len(patient_keys)):
        ...
        for j in range(len(data[patient])):
            x.append(data[patient][j])
            y.append(labels[patient][j])
    ...
    zeros = 1.-m/(512.**2)
    print((zeros, ones))
    print((1./zeros, 1./ones, zeros/ones))
